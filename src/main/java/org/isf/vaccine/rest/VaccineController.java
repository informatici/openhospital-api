...
public class VaccineController {
    ...
    public ResponseEntity<List<VaccineDTO>> getVaccines() throws OHServiceException {
        ...
        List<VaccineDTO> listVaccines = vaccines.stream().map(it -> modelMapper.map(it, VaccineDTO.class)).collect(Collectors.toList());
    ...
    public ResponseEntity newVaccine(@RequestBody VaccineDTO newVaccine) throws OHServiceException {
        ...
        boolean isCreated;
        try {
             isCreated = vaccineManager.newVaccine(modelMapper.map(newVaccine, Vaccine.class));
    ...
    public ResponseEntity updateVaccine(@RequestBody VaccineDTO updateVaccine) throws OHServiceException {
        ...
        boolean isUpdated = vaccineManager.updateVaccine(modelMapper.map(updateVaccine, Vaccine.class));
        if (!isUpdated) {
    ...
    public ResponseEntity deleteVaccine(@RequestBody VaccineDTO vaccineToDelete) throws OHServiceException {
        ...
        boolean isDeleted = vaccineManager.deleteVaccine(modelMapper.map(vaccineToDelete, Vaccine.class));
        if (!isDeleted) {
