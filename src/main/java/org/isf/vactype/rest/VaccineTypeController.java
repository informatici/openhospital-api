...
public class VaccineTypeController {
    ...
    public ResponseEntity<List<VaccineTypeDTO>> getVaccineType() throws OHServiceException {
        ...
        List<VaccineTypeDTO> listVaccines = vaccinesTypes.stream().map(it -> modelMapper.map(it, VaccineTypeDTO.class)).collect(Collectors.toList());
    ...
    public ResponseEntity newVaccineType(@RequestBody VaccineTypeDTO newVaccineType) throws OHServiceException {
        ...
        boolean isCreated;
        try {
            isCreated = vaccineTypeManager.newVaccineType(modelMapper.map(newVaccineType, VaccineType.class));
    ...
    public ResponseEntity updateVaccineType(@RequestBody VaccineTypeDTO updateVaccineType) throws OHServiceException {
        ...
        boolean isUpdated = vaccineTypeManager.updateVaccineType(modelMapper.map(updateVaccineType, VaccineType.class));
        if (!isUpdated) {
    ...
    public ResponseEntity deleteVaccineType(@RequestBody VaccineTypeDTO vaccineTypeToDelete) throws OHServiceException {
        ...
        boolean isDeleted = vaccineTypeManager.deleteVaccineType(modelMapper.map(vaccineTypeToDelete, VaccineType.class));
        if (!isDeleted) {
