...
public class BillController {
    ...
    ResponseEntity<FullBillDTO> newBill(@RequestBody FullBillDTO newBillDto) throws OHServiceException {
        ...
        Bill bill = modelMapper.map(newBillDto.getBill(), Bill.class);
        ...
        ArrayList<BillItems> billItems = new ArrayList<BillItems>(newBillDto.getBillItems().stream().map(item -> modelMapper.map(item, BillItems.class)).collect(Collectors.toList()));
        
        ArrayList<BillPayments> billPayments =  new ArrayList<BillPayments>( newBillDto.getBillPayments().stream().map(item -> modelMapper.map(item, BillPayments.class)).collect(Collectors.toList()));
    ...
    ResponseEntity<FullBillDTO> updateBill(@PathVariable Integer id, @RequestBody FullBillDTO odBillDto) throws OHServiceException {
        ...
        Bill bill = modelMapper.map(odBillDto.getBill(), Bill.class);
        ...
        ArrayList<BillItems> billItems = new ArrayList<BillItems>(odBillDto.getBillItems().stream().map(item -> modelMapper.map(item, BillItems.class)).collect(Collectors.toList()));
        
        ArrayList<BillPayments> billPayments =  new ArrayList<BillPayments>( odBillDto.getBillPayments().stream().map(item -> modelMapper.map(item, BillPayments.class)).collect(Collectors.toList()));
	...
	public ResponseEntity<List<BillDTO>> searchBills(@RequestParam(value="datefrom") @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") Date dateFrom,
        ...
        billDTOS = bills.stream().map(bil-> modelMapper.map(bil, BillDTO.class)).collect(Collectors.toList());
	...
	public ResponseEntity<List<BillPaymentsDTO>> searchBillsPayments(
        ...
        paymentsDTOS = payments.stream().map(pay-> modelMapper.map(pay, BillPaymentsDTO.class)).collect(Collectors.toList());
	...
	public ResponseEntity<List<BillPaymentsDTO>> getPaymentsByBillId(@PathVariable(value="bill_id") Integer id) throws OHServiceException {
        ...
        List<BillPaymentsDTO> paymentsDTOS = bills.stream().map(pay-> modelMapper.map(pay, BillPaymentsDTO.class)).collect(Collectors.toList());
	...
	public ResponseEntity<List<BillItemsDTO>> getItems(@PathVariable(value="bill_id")Integer id) throws OHServiceException {
        ...
        List<BillItemsDTO> itemsDTOS = items.stream().map(it-> modelMapper.map(it, BillItemsDTO.class)).collect(Collectors.toList());
	...
	public ResponseEntity<List<BillDTO>> searchBills(@RequestParam(value="datefrom") @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") Date dateFrom,
        ...
        BillItems billItem = modelMapper.map(billItemDTO, BillItems.class);
        ...
        List<BillDTO> billDTOS = bills.stream().map(bil-> modelMapper.map(bil, BillDTO.class)).collect(Collectors.toList());
	...
	public ResponseEntity<List<BillDTO>> searchBillsByPayments(@RequestBody List<BillPaymentsDTO> paymentsDTO) throws OHServiceException {
        ...
        ArrayList<BillPayments> billPayments = new ArrayList<BillPayments>(paymentsDTO.stream().map(pay-> modelMapper.map(pay, BillPayments.class)).collect(Collectors.toList()));
